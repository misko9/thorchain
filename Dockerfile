...
# then log output of file /code/bin/thornode
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/thornode \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/thornode | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/thornode /usr/bin/thornode
