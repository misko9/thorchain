...
HTTPS_GIT := github.com/rollchains/thornode.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=thornode \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/thornode ./cmd/thornode
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/thornode.exe ./cmd/thornode
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/thornode
...
draw-deps:
	...
	@goviz -i ./cmd/thornode -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c thornode --local -f chains.yaml
endif
...
set-testnet-configs:
	thornode config set client chain-id localchain-1
	thornode config set client keyring-backend test
	thornode config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | thornode keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | thornode keys add acc1 --recover`
